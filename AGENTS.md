...
- Preserve installed app data with an update installation (`adb install --user 0 -r`). Never uninstall or clear app data to work around an installation failure without explicit user approval.

- Always target Android user 0 explicitly when installing updates. Never install for all users or another profile.
